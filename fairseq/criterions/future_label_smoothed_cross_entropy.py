...
def future_loss(lprobs, target, ignore_index=None, reduce=True):
    if target.dim() == lprobs.dim() - 1:
        target = target.unsqueeze(-1)
...
class FutureLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def __init__(self, args, task):
        ...
        self.future_lambda = args.future_loss_lambda
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        lp = utils.log_softmax(net_output[1][0], dim=2)
        lp = lp.view(-1, lp.size(-1))
        fu_loss = future_loss(lp, target, ignore_index=self.padding_idx, reduce=reduce)
        return loss, nll_loss, fu_loss

    @staticmethod
    def reduce_metrics(logging_outputs) -> None:
        """Aggregate logging outputs from data parallel training."""
        fu_loss_sum = utils.item(sum(log.get('fu_loss', 0) for log in logging_outputs))
        ...
        metrics.log_scalar('fu_loss', fu_loss_sum / ntokens / math.log(2), ntokens, round=3)
