...
class TranslationTask(FairseqTask):
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False, num_updates=0):

        model.train()
        loss, sample_size, logging_output = criterion(model, sample, num_updates=num_updates)
        if ignore_grad:
            loss *= 0
