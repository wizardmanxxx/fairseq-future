...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(
        ...
        if is_flip:
            positions = masked_reverse(positions)
        if self.onnx_trace:
...
#     return xr.to(reserved)

def masked_reverse(x, pad=1.):
    mask = (x != pad).to(x).float()
    upper_tri = torch.triu(torch.ones(x.size(1), x.size(1))).to(x).float()
    lower_tri = upper_tri.transpose(0, 1).to(x).float()
    ones = torch.ones(x.size()).to(x).float()
    ones = ones.matmul(upper_tri) - 1.
    ones = ones * (1 - mask)
    # obtain re-arranged tensor for reversing the indices
    rev_indices = (mask.matmul(lower_tri) - 1) * mask  # reverse indices start with 0
    rearrange = rev_indices + ones
    rearrange_mat = torch.zeros(rearrange.size(0), rearrange.size(1), rearrange.size(1)).to(x)
    rearrange_mat = rearrange_mat.scatter(2, rearrange.long().unsqueeze(2), 1)
    xr = torch.bmm(rearrange_mat.float(), x.float().unsqueeze(2)).squeeze(-1)
    return xr.to(x)
